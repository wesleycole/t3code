...
export function buildThreadListV2Items(input: {
  ...
  for (const thread of input.threads) {
    ...
    // Specialist children remain reachable from their parent and through search,
    // but do not compete with user-created conversations in the default list.
    if (query.length === 0 && thread.specialist !== undefined) continue;
    if (
      query.length > 0 &&
