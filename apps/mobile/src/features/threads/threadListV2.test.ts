...
describe("buildThreadListV2Items", () => {
  it("hides specialist children from the default list but keeps them searchable", () => {
    const parentThreadId = ThreadId.make("parent");
    const parent = makeThread({ id: parentThreadId, title: "Parent conversation" });
    ...
    expect(
      buildThreadListV2Items({ ...input, searchQuery: "" }).items.map((item) => item.thread.id),
    ).toEqual([parentThreadId]);
    expect(
      buildThreadListV2Items({ ...input, searchQuery: "mobile navigation" }).items.map(
        (item) => item.thread.id,
      ),
    ).toEqual([child.id]);
  });
